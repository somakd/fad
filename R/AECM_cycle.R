...
# for N by P data matrix X
  ...
  ybar = fad:::RXM(X,ER)
  csd = fad:::mSD(X,mmu,D,ER,VR,ybar)
  start = (1/csd^2)*D
  start[start >= 1] = 1
  zz = fad:::fads.fit.rxmd(X,mmu,csd,ER,VR,ybar,q, start = start, lower = 1e-5)
